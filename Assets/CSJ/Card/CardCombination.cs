...
public static class CardCombination
{
    public static CardCombinationEnum CalCombination(List<MinorArcana> cards, out List<int> cardNums)
    {
        ...
        foreach (int _num in numbers)
        {
            ...
            if (_num == 2)
            {
                ...
                {
                    CheckOnePair(JokerNum, i, ref JokerUsedTriple, cardNums, IsComb, ref IsEnd);
                }
                else
                {
                    CheckTwoPair(JokerNum, i, ref JokerUsedTriple, cardNums, IsComb, ref IsEnd);
                }
            }
            #endregion
            ...
            {
                CheckTriple(JokerNum, i, cardNums, IsComb, ref IsEnd);
            }
            #endregion
            ...
            {
                CheckFourCard(JokerNum, i, cardNums, IsComb, ref IsEnd);
            }
            #endregion
        ...
        cardNums.Clear();
        for (int j = 0; j < JokerNum; j++)
        {
            cardNums.Add(0);
        }
        for (int rank = 0; rank < 13; rank++)
        {
            int count = numbers[rank];
        ...
        // 마지막으로 조커만 있는지 체크
        if (cardNums.Count != 0 && cardNums[0] == cardNums.Count)
        {
            JokerNum -= 1;
        }
        switch (JokerNum)
    ...
    private static void CheckStraight(int Joker, int straightLen, List<int> cardNum, int[] numbers, Dictionary<CardCombinationEnum, bool> IsComb, ref bool IsEnd)
    {
        ...
        int maxStart = AHigh.Length - straightLen + Joker;
